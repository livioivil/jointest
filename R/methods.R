...
# 
# 
print.jointest <- function (mods, p.values = c("raw", "adjusted"), 
                            mark_signif = 0.05) 
{
  library(wesanderson)
  cbPalette = wes_palette("Zissou1", 5, type = "continuous")[c(1, 
                                                               4, 5)]
  p.values = p.values[1]
  y = "-log10(p.vals)"
  x = "Estimate"
  ...
  if (p.values == "raw") 
    title = "(Raw) p-values"
  else title = "Adjusted p-values"
  p <- ggplot(D, aes_string(y = y, x = x, group = group, color = group)) + 
    scale_colour_manual(values = cbPalette) + geom_point(aes(shape = is_signif)) + 
    ggtitle(title)
  if (!(mark_signif %in% c(0, 1))) {
    p <- p + scale_shape_manual(values = c(3, 19), name = paste(p.values, 
                                                                "p-value"), labels = paste0(c("p >  ", 
                                                                                              "p <= "), mark_signif))
  }
  p
}
...
summary.jointest <- function (object, ...) 
{
  if (!is.null(object[[1]]$summary_table)) {
    ...
    tab = cbind(Model = names(object[1]), Coeff = rownames(tab), 
                tab)
    summary_table = tab
    if (length(object) > 1) {
      for (i in 2:length(object)) {
        ...
        tab = tab[!is.na(tab[, "Score"]), ]
        tab = cbind(Model = names(object[i]), Coeff = rownames(tab), 
                    tab)
